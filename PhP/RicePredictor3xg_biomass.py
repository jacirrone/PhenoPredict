...
def main(*args):
	...
	if rice_data:
		...
		phenotype_data = pd.read_csv("./data/test_phenotype.csv")
	...
	if rice_data:
		...
		geneexpression_data = pd.read_csv("./data/test_data.csv")
	...
	if rice_data:
		...
		lab_data = pd.read_csv("./data/training_data.csv")
	...
	for l in range(0, number_seeds):
		...
		if rice_data:
			for g_id, genotype in enumerate(genotypes):
				...
				y_test_biomass_field = phenotype_data.loc[genotype, "Biomass"]
			...
			field_genexp_data_single_genotype = geneexpression_data.loc[:, genes_features_filtered]#indx_phenotype_notna, genes_features_filtered]#field_indxes_nipp, genes_features_filtered]
			y_test_biomass_field = phenotype_data.loc[:, "Biomass"]
			X_test = field_genexp_data_single_genotype
	...
	df_corr2.to_csv("df_correlation_BiomassPredictions_"+ensemble_method+str(ntrees)+"Trees.txt", sep='\t')

	X_for_corr = X.copy()
	X_for_corr["Biomass"] = y
	pairwaise_corr = X_for_corr.corr()
	#gene_biomass_corr_sign = np.sign(pairwaise_corr["Biomass"][:-1].values)
	gene_biomass_corr = pairwaise_corr["Biomass"][:-1].copy()
	gene_biomass_corr = gene_biomass_corr.reset_index()
	gene_biomass_corr["Sign"] = np.sign(gene_biomass_corr["Biomass"])
	gene_biomass_corr.columns = ["Gene", "CorrelationCoeffWBiomass", "Sign"]
	gene_biomass_corr.to_csv("GenesCorrelationWBiomass_LabData.txt", sep="\t")
